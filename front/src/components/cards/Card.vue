<template>
  <div class="Card">
    <div>
      <div class="content">
        新华社北京3月16日电
        题：始终坚持人民至上——二论学习贯彻习近平主席在十四届全国人大一次会议上的重要讲话
        新华社评论员
...
//请无视onmounted里的逻辑 这只是一个传参示例 清空.content然后添加一个图片
onMounted(() => {
  const odom = document.querySelector(".content");

  if (props.msg) {
    ((odom as unknown) as HTMLElement).innerHTML = "";
    ((odom as unknown) as HTMLElement).innerHTML = `${props.msg}`;
  }
});
</script>
...
.Card {
  ...
  border-radius: 5px;
  padding-bottom: 100%;
  background-image: radial-gradient(circle at center, rgba(0, 242, 255, 0.868) 0, rgba(248, 185, 209, 0.742) 76%);
  position: relative;
  > div {
    padding: 10px;
